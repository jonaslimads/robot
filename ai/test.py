...
def draw_boxes(boxes, confidences, class_ids, classes, img, colors, confidence_threshold, NMS_threshold, output_image_path=None):
...
def detect_image_file(image_path, output_image_path, yolo_weights, yolo_cfg, coco_names, confidence_threshold, nms_threshold):
    ...
    draw_boxes(boxes, confidences, class_ids, classes, img, colors, confidence_threshold, nms_threshold, output_image_path)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--output', help='Path to the output images', default=None)
    ap.add_argument('--camera', help='To use the live feed from web-cam', type=bool, default=False)
    ap.add_argument('--weights', help='Path to model weights', type=str, default=f'{path}/yolov3.weights')
    ap.add_argument('--configs', help='Path to model configs',type=str, default=f'{path}/yolov3.cfg')
    ap.add_argument('--class_names', help='Path to class-names text file', type=str, default=f'{path}/coco.names')
    ...
    if image_path and output_image_path:
        print(image_path)
        print(output_image_path)
        detect_image_file(image_path, output_image_path, yolo_weights, yolo_cfg, coco_names, confidence_threshold, nms_threshold)
